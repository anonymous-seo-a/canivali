...
export function ArticlesView() {
  ...
  const filtered = q
    ? items.filter((a) => a.title.toLowerCase().includes(q.toLowerCase()) || String(a.article_id).includes(q))
    : items;

  return (
    <>
      <h2>記事インベントリ ({total})</h2>
      <div className="toolbar">
        <input placeholder="title / id 検索" value={q} onChange={(e) => setQ(e.target.value)} />
